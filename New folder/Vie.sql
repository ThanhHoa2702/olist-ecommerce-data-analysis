...
FROM staging_orders; -- Do dữ liệu hoạt động phiên giao dịch việc đếm customer_id, 
					 -- đặt tên unique_customer_id đảm bảo rằng mỗi mã đơn hàng đồng nghĩa 1 mã khách hàng (Mối quan hệ 1 - 1) trong bảng Orders
...
FROM staging_orders; -- Kiểm tra giá trị trạng thái đơn hàng: Nếu Trạng thái đơn hàng NULL => Lỗi dữ liệu
					 -- Kiểm tra giá trị thanh toán khách hàng: Nếu xuất hiện NULL ở thời gian khách hàng => Lỗi dữ liệu, hệ thống 
...
--- Tổng hợp các trạng thái đơn treo trong Warehouse
SELECT 
	order_status,
	COUNT(DISTINCT o.order_id)
FROM orders AS o
...
--- Tổng nguồn tiền rủi ro trong Warehouse 
SELECT 
...
--- Tổng hợp các đơn hàng treo và chi phí vận chuyển rủi ro trong Shipping 
SELECT 
...
ON o.order_id = items.order_id
WHERE order_status = 'shipped'
	AND ((SELECT MAX(delivered_customer_date) FROM orders)::DATE - estimated_delivery::DATE > 30) -- Vi phạm mốc thời gian: Thời gian cập nhật dữ liệu cuối cùng của hệ thống vượt qua thời gian dự kiến > 30 days
...
-- Truy vấn 141 đơn hàng bị huỷ trước khi xác nhận đơn (Pre-Payment)
...
ON o.order_id = items.order_id
WHERE order_status = 'canceled' 
	AND approved_at_date IS NULL -- Không có thời gian xác nhận đơn 
	AND items.order_id IS NULL -- Không có thông tin

-- Truy vấn 626 đơn hàng không có thông tin sản phẩm (No Info) Ware House
